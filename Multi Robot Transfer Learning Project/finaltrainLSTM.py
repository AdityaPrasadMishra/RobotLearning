# -*- coding: utf-8 -*-
"""
Created on Mon Apr 24 23:20:25 2017
...
from keras.layers import Lambda,Activation, Flatten
...
def generate_batches(X,Yarray, batch_size, istraining):
    #print("yarray")
    #print(Yarray)
    imagessuper = numpy.empty([batch_size,5,3,40, 40])
    while True:
        i = 0
        TorqueDiff = []
        for index in numpy.random.permutation(X.shape[0]):
            ...
            for j in range(10):
                if j%2==0:
                    if istraining:
                        image =  PIL.Image.open(r"G:\VRepPython\finale\imageslooper1_"+str(X[index-j])+".jpeg")
                        ...
                        image = image.astype('float32')
                        image = image/255.0
                    images.append(numpy.transpose(image))
            TorqueDiff1 = Yarray[X[index]]
            #print(TorqueDiff1)
            TorqueDiff.append(TorqueDiff1)
            #print(len(images))
            imagessuper[i]= numpy.array(images)
            i += 1
            if i == batch_size:
                #print(TorqueDiff)
                ...
                #print(len(images))
                break
        yield imagessuper, numpy.array(TorqueDiff)
...
# fix random seed for reproducibility
    ...
    model = Sequential()
    model.add(ConvLSTM2D(nb_filter=15, nb_row=3, nb_col=3, input_shape=(5,3,40,40),
